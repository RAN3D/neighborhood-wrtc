...
class Neighborhood extends EventEmitter {
    ...
    _connect (protocolId, arg1, arg2){
        if (typeof arg1 === 'function' && typeof arg2 === 'undefined') {
            ...
            this._finalize(arg1); // arg1: response
        };
    };

    /**
     * @private
     * Initiate the creation of a WebRTC connexion. At this point, the identity
    ...
    _initiate (protocolId, sender) {
        ...
        socket.on('signal', (offer) => {
            sender(new MRequest(entry.tid,this.ID,offer));
        });
    ...
    /**
     * @private
     * Try to finalize the WebRTC connexion using the remote offers.
     * @param {string} protocolId The identifier of the protocol that wishes to
     * open a connexion.
     ...
     **/
    _answer (protocolId, sender, msg) {        
        // #1 initialize the entry if it does not exist
        let firstCall = false;
        if (!this.pending.has(msg.tid)) {
            firstCall = true;
            let entry = new EPending(uuid(), msg.peer, protocolId);
        ...
        // #2 check if a WebRTC connexion to peerId already exists
        let entry = this.pending.get(msg.tid);
        if (entry.alreadyExists || entry.successful) { return ;};
