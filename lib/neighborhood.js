...
class Neighborhood extends EventEmitter {
    ...
    /**
     * Disconnect one of the arcs with the identifier in argument. If
    ...
    initiate (callbacks, protocol) {
        ...
        socket.on('signal', (offer) => {
            entry.onOffer && entry.onOffer(MRequest(entry.id, self.ID, offer, protocol));
        });
    ...
    accept (message, callbacks) {
        ...
        socket.on('signal', function (offer) {
            entry.onOffer && entry.onOffer(MResponse(entry.id, self.ID, offer, entry.protocol));
        });
    ...
    alreadyExists (message, callbacks) {
        ...
        if      (!alreadyExists) {
            ...
            if (toRemove && toRemove.socket) { // exists but socket still w8in
                ...
                message.offer && callbacks && callbacks.onAccept && callbacks.onAccept(MResponse(message.tid, this.ID,  null,   message.protocol));
