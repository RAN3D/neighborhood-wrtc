...
const ELiving = require('./entries.js').ELiving;

const ExSocketNotFound = require('./exceptions.js').ExSocketNotFound;
...
class ArcStore {
    ...
    remove (peerId, protocolId) {
        ...
        if (!this.store.has(peerId).decrement(protocolId)){
            throw new ExSocketNotFound(
                'arcStore',
                peerId,
                protocolId,
     ...
     * @param {string} protocolId The identifier of the protocol that wishes to
     * remove all its arcs.
     * @returns {Array of ELiving} All the entries that no protocol is using any
